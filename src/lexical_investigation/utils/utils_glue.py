...
class InputFeatures(object):
    ...
    def __init__(self, tokenised_sent, input_ids, input_mask, segment_ids, label_id):
        self.tokenised_sent = tokenised_sent
...
class RQEProcessor(DataProcessor):
    ...
    def _create_examples(self, lst, set_type):
        ...
        for _id, key in tqdm(enumerate(lst), desc="Preparing Data", total=len(lst)):
            ...
            text_a = data["data"][0]
            text_b = data["data"][1]
            label = int(data["label"]) if set_type != "test" else 0
            assert label in self.get_labels()
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label)
            )
...
def convert_examples_to_features(examples, label_list, max_seq_length,
    ...
    for (ex_index, example) in tqdm(enumerate(examples), total=len(examples)):
        ...
        features.append(
                InputFeatures(tokens, input_ids=input_ids,
